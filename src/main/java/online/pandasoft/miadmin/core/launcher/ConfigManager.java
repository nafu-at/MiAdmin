...
public class ConfigManager {
    ...
    private final File configFile = new File("MiConfig.yaml");
    ...
    public boolean existsConfig(boolean generateFile) {
        ...
        if (!result && generateFile) {
            try (InputStream original = ClassLoader.getSystemResourceAsStream("MiConfig.yaml")) {
    ...
    public void reloadConfig() {
        try (FileInputStream configInput = new FileInputStream(configFile)) {
            miConfig = MAPPER.readValue(configInput, MiConfig.class);
