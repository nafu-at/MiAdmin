...
public class SilenceCommand extends CommandExecutor {
    private static final MiAdminLauncher launcher = Main.getLauncher();

    public SilenceCommand(String name, String... aliases) {
        super(name, aliases);
    }
        ...
        if (context.getArgs().length >= 1) {
            for (String username : context.getArgs()) {
                ...
                if (user == null) {
                    ...
                    context.printMessage(Level.INFO, "{} is already silenced.", user.getUsername());
                } else {
                    AdminSilenceSuspendParameter suspendParameter = (AdminSilenceSuspendParameter) RequestParameters.newInstance(RequestParameters.ADMIN_SILENCE_USER);
                    ...
                    HttpRequestTask requestTask = new HttpRequestTask(suspendParameter, new MiTaskResultHandler() {
                        ...
                        public void onTaskSuccess(MiTask executedTask, MiTaskResult result) {
                            ...
                            if (requestResult.getCode() == suspendParameter.getSuccessCode()) {
                                context.printMessage(Level.INFO, "{} has been silence.", user.getUsername());
                            }
                        }
    ...
    public String getDescription() {
        return "Silence the user.";
    }

    @Override
    public String getHelp() {
        return getName() + " <username>\n----\n" +
                "<username>: Silences the specified user.\n";
    }
}
